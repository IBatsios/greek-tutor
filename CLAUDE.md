...
- `app/` — `auth` (argon2, DB sessions), `tutor` (session loop), `srs` (SM-2),
  ...
  (router), `harada_export` (the board payload + mirror file, pure).
...
FOCUS prompt block, the board UI on `/`, the versioned export the tracker
mirrors). Step 5 (routine sheet) is deferred to the tracker by decision. **The
app has not yet been run end to end against the real Claude API.** Next: cycle
close (step 6). Details and known defects: `docs/handoff-items/handoff-next-phase.md`.
